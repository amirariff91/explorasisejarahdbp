...
export default function MultipleChoiceQuestion({ question, onAnswer }: Props) {
  ...
  const textBoostStates = [
    "selangor",
    "kuala-lumpur",
    "negeri-sembilan",
  ] as const;
  const isTabletTextBoost =
    isTablet &&
    textBoostStates.includes(
      question.state as (typeof textBoostStates)[number],
    );
  const isNegeriSembilan = question.state === "negeri-sembilan";
  const isTightPhone =
    isPhone && (isSabah || isSarawak || isMelaka || isSelangor);
  const forceWrap = isTightPhone;
  const questionMaxWidth = forceWrap
    ? "72%"
    : isTabletTextBoost
      ? "84%"
      : "85%";
  ...
  const questionFontSizeRaw = forceWrap
    ? Math.round(baseQuestionFontSize * 1.08)
    : baseQuestionFontSize;
  const questionFontSize = Math.round(
    questionFontSizeRaw * tabletQuestionBoost,
  );
  const baseOffsets = getQuestionOffsets(
  ...
  const isLargeBoardPhone =
    isPhone && largeBoardStates.includes(question.state as any);
  const isLargeBoardTablet =
    !isPhone && largeBoardStates.includes(question.state as any);
  ...
  const boardSize = {
    width:
      ...
      tabletBoardMultiplier,
    height:
      baseBoardSize.height *
      boardSizeMultiplier *
      phoneBoardMultiplier *
      tabletBoardMultiplier,
  };
  ...
  const offsets = isTightPhone
    ? {
        ...
        optionsContainer: {
          ...
          gap: Math.min(baseOffsets.optionsContainer.gap, 12),
        },
        optionRow: {
          ...baseOffsets.optionRow,
          gap: Math.min(baseOffsets.optionRow.gap, 8),
        },
      }
    : {
        ...baseOffsets,
  ...
  const nextButtonSize = isLandscape
    ? ButtonSizes.next.landscape
    : ButtonSizes.next.portrait;
  ...
  const answerFontSize =
    (isTabletTextBoost ? baseAnswerFont * 1.3 : baseAnswerFont) * 0.92; // slight trim for overflow safety

  // Switch to 4×1 block layout when any option exceeds 3 words
  const useBlockLayout = question.options.some(
    (opt) => opt.trim().split(/\s+/).length > 3,
  );
  const rowCount = useBlockLayout ? 4 : 2;
  ...
  const minAnswerHeight =
    answerFontSize * Typography.lineHeight.tight * (useBlockLayout ? 1 : 2) +
    answerHorizontalPadding * 2;

  // Available vertical space for the answer grid inside the board.
  ...
  // Include gap between rows.
  const totalAnswerRowGaps =
    offsets.optionsContainer.gap + offsets.optionRow.gap;
  const maxHeightPerAnswerFromBoard =
    (availableAnswerAreaHeight - totalAnswerRowGaps) / rowCount;
  ...
  let clampedButtonHeight =
    Math.max(baseCSSHeight, minAnswerHeight) * phoneHeightTrim;

  if (
    Number.isFinite(maxHeightPerAnswerFromBoard) &&
    maxHeightPerAnswerFromBoard > 0
  ) {
    clampedButtonHeight = Math.min(
      clampedButtonHeight,
    ...
    <View style={styles.container}>
      <View
        style={[
          ...
          { marginTop: isTablet ? 0 : getResponsiveSizeScaled(10, width) },
        ]}
      >
        <ImageBackground
        ...
        >
          ...
          <View
            style={[
              styles.questionSection,
              {
                ...
                marginBottom: offsets.answerAreaTop,
          ...
          >
            <Text
              ...
              numberOfLines={forceWrap || isNegeriSembilan ? 2 : 7}
          ...
          <View
            style={[
              ...
              { transform: [{ translateY: answerYOffset }] },
            ]}
          >
            <View
              style={[
                styles.optionsContainer,
                { gap: offsets.optionsContainer.gap },
              ]}
            >
              {useBlockLayout ? (
                question.options.map((option, i) => (
                  <View key={i} style={[styles.optionRow, { gap: 0, width: "100%" }]}>
                    {renderOptionButton(option, i, false)}
                  </View>
                ))
              ) : (
                <>
                  <View style={[styles.optionRow, { gap: offsets.optionRow.gap }]}>
                    {question.options
                      .slice(0, 2)
                      .map((option, i) => renderOptionButton(option, i, false))}
                  </View>

                  <View style={[styles.optionRow, { gap: offsets.optionRow.gap }]}>
                    {question.options
                      .slice(2, 4)
                      .map((option, i) => renderOptionButton(option, i, true))}
                  </View>
                </>
              )}
            </View>
