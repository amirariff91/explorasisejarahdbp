...
export default function FillBlankQuestion({ question, onAnswer }: Props) {
  ...
  const isLargeBoardPhone =
    isPhone && largeBoardStates.includes(question.state);
  ...
  const perakTabletBoardBoost =
    !isPhone && question.state === "perak" ? 1.3 : 1.0;
  ...
  let boardWidth =
    ...
    perakTabletBoardBoost *
    perlisTabletScaleDown;
  let boardHeight =
  ...
  const maxWidthPercent = isPhone ? 0.9 : isPerlisTablet ? 0.72 : 0.75;
        ...
        >
          ...
          <View style={styles.buttonWrapper}>
            <Pressable
              style={({ pressed }) => [
                {
                  transform: [{ scale: pressed && answer.trim() ? 0.95 : 1 }],
                },
              ]}
